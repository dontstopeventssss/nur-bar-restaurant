...
export default function BarPage() {
  ...
  const loadNotifications = useCallback(async () => {
    ...
    if (error) {
      console.error('Errore caricamento notifiche bar', error);
  ...
  const clearCompleted = async () => {
    ...
    const { data, error } = await supabase.rpc('clear_bar_completed_notifications');

    console.log('RPC clear bar result:', data);

    if (error) {
      console.error('Errore pulizia completati bar', error);
      setErrorMessage('Impossibile pulire i completati bar.');
      setClearing(false);
      return;
    }
              ...
              pendingOrders.map((item, index) => (
                ...
                >
                  <div style={styles.orderMeta}>
                    {index === 0 && (
                      <span style={styles.badgeFirst}>⚡ PRIMO</span>
                    )}
